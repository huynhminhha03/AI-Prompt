...
class CozeController {
        ...
        try {
            ...
            for await (const part of stream) {
                ...
                if (event === ChatEventType.CONVERSATION_MESSAGE_DELTA) {
                    ...
                    const wallet = await Wallet.findOne({ where: { user_id: conversation.user_id } });
        ...
        try {
            ...
            for await (const part of stream) {
                ...
                if (part.event === ChatEventType.CONVERSATION_MESSAGE_DELTA) {
                    ...
                    res.write(`data: ${JSON.stringify(part.data.content)}\n\n`);
                }
    ...
    countConversations(req, res) {
        ...
        Conversation.count({ where: { user_id } }).then(count => {
            res.json({ count });
        }).catch(error => {
    ...
    getBalance(req, res) {
        const user_id = req.user.id;
        Wallet.findOne({ where: { user_id } }).then(wallet => {
